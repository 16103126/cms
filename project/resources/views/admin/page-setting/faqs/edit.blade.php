...
@section('title', __('Edit Faqs Page'))
        ...
        <div class="row">
            ...
            <div class="col-sm-10">
                <nav aria-label="breadcrumb">
                    ...
                    <li class="breadcrumb-item {{ (Request::is('admin/page/setting/faqs/edit*')) ? 'active' : '' }}">
                        <a href="{{ route('admin.page.setting.faqs.edit', $page->id) }}">@lang('Edit Page')</a>
                    </li>
                    </ol>
                </nav>
                <div class="card mb-4">
                    ...
                    <div class="card-body mt-5">
                        <div class="row">
                            <div class="col-sm-1"></div>
                            <div class="col-sm-10">
                                <form method="POST" action="{{ route('admin.page.setting.faqs.update', $page->id) }}">
                                    @csrf
                                    ...
                                    @php
                                        $faqs = json_decode($page->description, true);
                                    @endphp
                                    <div id="addFaqs">
                                        @foreach ($faqs as $question => $answer)
                                        <div class="removeFaqs">
                                            <div class="row mb-2">
                                                <label class="col-sm-3 col-form-label" for="basic-default-name">@lang('Question')</label>
                                                <div class="col-sm-9">
                                                    <input type="text" name="question[]" class="form-control" id="basic-default-name" value="{{ $question }}" required>
                                                </div>
                                            </div>
                                            <div class="row mb-3">
                                                <label class="col-sm-3 col-form-label" for="basic-default-name">@lang('Answer')</label>
                                                <div class="col-sm-9">
                                                    <textarea name="answer[]" rows="5" class="form-control" required>{{ $answer }}</textarea>
                                                </div>
                                            </div>
                                            <div class="row mb-3">
                                                <label class="col-sm-11 col-form-label" for="basic-default-name"></label>
                                                <div class="col-sm-1">
                                                <button type="button" class="btn btn-sm btn-danger removeFaqsBtn" id="removeFaqsBtn"><i class='bx bx-minus'></i></button>
                                                </div>
                                            </div>
                                        </div>
                                        @endforeach
...
<script>
    ...
    $(document).on('click', '.removeFaqsBtn', function(){
        $(this).parents('.removeFaqs').remove();
    });
